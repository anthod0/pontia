...
# - [x] Milestone 8：Generic Client Adapter Contract 验证

**状态：已完成**

完成时间：2026-04-25

完成摘要：已新增 generic adapter contract 验证替身，明确 MVP capability model（`accept_task`、`report_turn_started`、`report_turn_finished`、`interrupt`、`stream_output`、`heartbeat`、`artifact_sources`），并让 generic runtime binding 从 adapter capability 映射到 External API SessionView。Turn input 下发会记录 Control Plane 分配的 `session_id` / `turn_id`，Internal Event API 继续作为 adapter 回传 turn facts 的统一入口，新增 artifact source registration service 以注册已知 `file://` artifact source，并通过 M8 集成测试覆盖能力降级、事件回传、artifact 读取和 client-specific 字段隔离。
...
# - [x] Milestone 9：MVP 端到端编排验收

**状态：已完成**

完成时间：2026-04-27

完成摘要：已新增 M9 端到端验收集成测试，覆盖 backend-only HTTP polling 编排闭环：External API 创建 session、提交 turn、Internal Event API 回传 adapter facts、查询 turn / events / artifacts、读取 artifact content、unsupported interrupt capability 降级，以及 terminate session。补齐 capability unavailable 的 External API 状态码为 422，并验证认证失败、invalid request、not found、state conflict、capability unavailable 和写操作幂等语义。README 已补充 MVP 端到端验收说明。
