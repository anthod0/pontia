...
#[tokio::test]
async fn sqlite_session_projection_upsert_preserves_updated_at_for_state_version_only_changes() {
    let pool = test_pool().await;
    ...
    SqliteSessionRepository::upsert_projection_in_tx(
        ...
        SessionProjectionUpsertRecord {
            session_id: "sess_stable_updated_at".to_string(),
            client_type: "pi".to_string(),
            title: Some("Worker".to_string()),
            handle: Some("worker".to_string()),
            ...
            current_turn_id: Some("turn_stable_updated_at".to_string()),
            state_version: 10,
    ...
    tx.commit().await.expect("commit initial tx");

    let sentinel_updated_at = "2000-01-01T00:00:00.000Z";
    sqlx::query("UPDATE sessions SET updated_at = ? WHERE session_id = ?")
        .bind(sentinel_updated_at)
        .bind("sess_stable_updated_at")
        .execute(&pool)
        .await
    ...
    .await
    .expect("state-version-only upsert");
    tx.commit().await.expect("commit update tx");

    let row = SqliteSessionRepository::new(pool.clone())
        .load_projection_rows("sess_stable_updated_at")
        .await
        .expect("load projection")
        .into_iter()
        .next()
        .expect("projection exists");
    assert_eq!(row.state_version, 11);

    let updated_at: String =
        sqlx::query_scalar("SELECT updated_at FROM sessions WHERE session_id = ?")
            .bind("sess_stable_updated_at")
            .fetch_one(&pool)
            .await
            .expect("load updated_at");
    assert_eq!(updated_at, sentinel_updated_at);
}
