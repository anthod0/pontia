<script lang="ts">
  ...
  const idleAgentCount = $derived(activeAgents.filter((agent) => agent.state === 'idle').length)
...
<section aria-labelledby="agent-overview-title" class="mx-auto w-full max-w-4xl space-y-4">
  <div class="flex items-start justify-between gap-4">
    <div class="min-w-0">
      <h1 id="agent-overview-title" class="text-xl font-semibold tracking-tight">Overview</h1>
      <p class="mt-1 text-sm text-muted-foreground">{idleAgentCount} agent wait for next step</p>
    </div>
    <Select.Root
      type="single"
      value={selectorValue}
  ...
  <Card.Root class="gap-0 py-0">
    ...
    {:else if visibleAgents.length}
      ...
      {/if}
    {:else}
      <Empty.Root class="min-h-36 border-0 py-6">
        <Empty.Header>
          <Empty.Media variant="icon"><Bot /></Empty.Media>
          <Empty.Title>No active agents</Empty.Title>
          <Empty.Description>Start a chat below to launch an agent.</Empty.Description>
        </Empty.Header>
      </Empty.Root>
    {/if}
  </Card.Root>
