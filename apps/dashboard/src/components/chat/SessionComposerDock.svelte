  ...
  <div class="mx-auto w-full max-w-7xl">
    <div role="group" aria-label="Session status and controls" class="mb-2 flex min-w-0 items-center justify-between gap-2 px-2">
      <div class="flex min-w-0 flex-1 items-center gap-2">
        <Badge variant="outline" class={`h-7 w-7 shrink-0 px-0 ${sessionStateBadgeClass(session.state)}`} aria-label={`Session state: ${session.state}`} title={`Session state: ${session.state}`}>
          <SessionStateIcon class={sessionStateIconClass(session.state)} aria-hidden="true" />
        </Badge>
