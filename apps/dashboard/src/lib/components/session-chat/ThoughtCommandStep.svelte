...
<Collapsible.Root bind:open class="relative min-w-0">
  ...
  >
    <span class="relative z-10 flex size-6 shrink-0 items-center justify-center bg-background text-muted-foreground">
      <SquareTerminal class="size-4 group-hover/command:hidden" aria-hidden="true" />
      <ChevronRight class={cn('hidden size-4 transition-transform group-hover/command:block', open && 'rotate-90')} aria-hidden="true" />
    </span>
  ...
  <Collapsible.Content>
    <div class="mb-3 ml-9 max-w-[calc(100%_-_2.25rem)] overflow-x-auto">
      <div class="command-code min-w-full text-sm">{@html highlightedCommand}</div>
    </div>
  </Collapsible.Content>
</Collapsible.Root>
...
<style>
  ...
  .command-code :global(pre.shiki span) {
    color: var(--shiki-light, inherit);
  }
