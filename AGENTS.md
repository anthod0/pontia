...
- External API state must come from the event store/projections. Do not treat tmux state, runtime logs, client internals, or workspace files as authoritative.
...
- Keep client-specific behavior inside adapter/runtime/client-plugin boundaries (`control/runtime/`, `control/agent-clients/`, `control/application/`, `control/http/`, `clients/*/`). Do not leak client-specific fields into generic domain events or External API view models.
- Client turn output/completion/failure must be reported by hooks through the Internal Event API. Do not parse TUI screen contents, runtime logs, or tmux process exit as turn completion facts.
...
- Hook input-source agnostic rule: after input reaches the real agent TUI, hooks must treat all user prompts identically. Web UI input, backend-dispatched input, tmux paste/send-keys, and manually typed TUI input must flow through the same hook turn lifecycle path. Hook/client-plugin code must not branch on whether a prompt originated from Web UI or manual TUI. Backend pending/current-turn context may provide correlation metadata only; it must not be required for hooks to create/report a turn.
