...
Reports confirmed Claude Code turn facts to llmparty through `/internal/v1/events`.
...
The hook reads `LLMPARTY_CURRENT_TURN_FILE`, posts to `LLMPARTY_INTERNAL_EVENT_URL` or the context file URL, and writes JSONL diagnostics to `LLMPARTY_CLAUDE_HOOK_LOG`.
