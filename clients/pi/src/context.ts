...
export async function loadTurnContext(env: EnvLike = process.env): Promise<LoadTurnContextResult> {
  ...
  const runtimeInstanceId = optionalString(env.PILOTFY_RUNTIME_INSTANCE_ID) ?? optionalString(record?.runtime_instance_id);
  ...
  if (!runtimeInstanceId) errors.push("runtime_instance_id or PILOTFY_RUNTIME_INSTANCE_ID is required");
