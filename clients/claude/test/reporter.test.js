...
describe("EventReporter", () => {
    test("sends a reported fact and returns pontia's canonical turn_id", async () => {
        ...
        const reporter = new EventReporter({ fetch: fetchImpl, logFile: "/tmp/unused-claude-hook.log" });
        const result = await reporter.report({ internalEventUrl: "http://localhost/internal/v1/events" }, {
            session_id: "sess_1",
            type: "turn.started",
            data: {
                runtime_instance_id: "rtinst_1",
