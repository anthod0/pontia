...
async function reportReadyForManagedSession(input: ClaudeHookInput, deps: RequiredDeps): Promise<void> {
  ...
  if (!loaded.ok) return;
  const context: SessionContext = { ...loaded.context, ...sessionDetailsFromHook(input) };
  await deps.makeReporter(loaded.logFile).report(context, buildSessionReadyEvent(context));
}

async function reportReadyForManualSession(input: ClaudeHookInput, deps: RequiredDeps): Promise<void> {
...
async function manualTurnContext(input: ClaudeHookInput, deps: RequiredDeps, logFile: string): Promise<TurnContext | undefined> {
  ...
  if (existing?.ok) {
    ...
    if (session) await deps.makeReporter(logFile).report(session, buildSessionReadyEvent(session));
  } else if (existing) {
    await deps.logDiagnostic(logFile, { level: "warn", code: "session_context_lookup_failed", message: existing.reason });
  }
