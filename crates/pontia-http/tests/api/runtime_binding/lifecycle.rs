...
async fn retrying_an_old_terminal_fact_does_not_end_the_current_turn() {
    ...
    let (completed_status, completed) = crate::common::reporting::report_fact(
        state.clone(),
        json!({
            "session_id": session_id,
    ...
    let (second_started_status, second_started) = crate::common::reporting::report_fact(
        state.clone(),
        json!({
            "session_id": session_id,
            "type": "turn.started",
    ...
    let (retry_status, retry) = crate::common::reporting::report_fact(
        state.clone(),
        json!({
            "session_id": session_id,
            "turn_id": first_turn_id,
